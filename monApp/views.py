...
def importerCSV():
    ...
    if file and file.filename.endswith(".csv"):
        ...
        # On supprime l'ancien JSON pour éviter qu'il ne revienne hanter le nouveau CSV
        json_path = os.path.join(UPLOAD_FOLDER, "configuration.json")
        if os.path.exists(json_path):
            os.remove(json_path)

        # On vide la session pour forcer un rechargement propre
...
def importerJSON():
    ...
    if file and file.filename.endswith(".json"):
        ...
        # On vide la session pour forcer le code à lire le fichier JSON
        session.pop("dico_importance", None)
        session.pop("criteres_groupes", None)
        session.pop("nb_groupes", None)
